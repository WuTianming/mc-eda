...
def put_nor_array(x, y, z, floor_plan=None):
    ...
    with Offset(x, y, z):
        for d in range(depth):
            ...
            if spiral_type == 0:
                with Offset(1, 0, 1, 0):
                    put_nor_layer_0(0, -2*d, 0, wires, i_len, o_len, inverter_mask, tmap)
            elif spiral_type == 1:
                with Offset(-2, 0, 1, 1):
                    put_nor_layer_0(0, -2*d, 0, wires, i_len, o_len, inverter_mask, tmap)
                # with Offset(0, 0, 0, 1):
                    # put_nor_layer_0(0, -2*d, 3, wires, i_len, o_len, inverter_mask, tmap)
            elif spiral_type == 2:
                with Offset(-2, 0, -2, 2):
                    put_nor_layer_0(0, -2*d, 0, wires, i_len, o_len, inverter_mask, tmap)
                # with Offset(0, 0, 0, 2):
                    # put_nor_layer_0(3, -2*d, 3, wires, i_len, o_len, inverter_mask, tmap)
            elif spiral_type == 3:
                with Offset(1, 0, -2, 3):
                    put_nor_layer_0(0, -2*d, 0, wires, i_len, o_len, inverter_mask, tmap)
                # with Offset(0, 0, 0, 3):
                #     put_nor_layer_0(3, -2*d, 0, wires, i_len, o_len, inverter_mask, tmap)
